...
func (h *Env) Init(appName, ConfigFile string) {
	...
	//基础
	h.ConfigPath = filepath.Join(EnvClient.RootPath, `config`, EnvClient.AppName)
	//配置初始化
	ConfigViper.AddConfigPath(h.ConfigPath)
	ConfigViper.SetConfigName(h.ConfigFile)
	ConfigViper.SetConfigType(`ini`)
	if readErr := ConfigViper.ReadInConfig(); readErr != nil {
		panic(readErr.Error())
	}
	h.PkgPath = filepath.Join(h.RootPath, `internal`, `pkg`)
	h.LogPath = filepath.Join(h.RootPath, `logs`)
	//webkit
	h.NodePath = gstool.SReplaces(ConfigViper.GetString(`path.webkit_node_path`), map[string]string{
		`{PKG_PATH}`: h.PkgPath,
	})
	//base配置初始化
	h.ConfigBase = &Base{
		DbFileName: ConfigViper.GetString(`base.dbFileName`),
		DbPath:     ConfigViper.GetString(`base.dbPath`),
		WebPath:    ConfigViper.GetString(`base.webPath`),
	}
	//web
	...
	h.WebkitDriverPath = ConfigViper.GetString(`path.webkit_driver_path`)
	h.WebkitDataPath = ConfigViper.GetString(`path.webkit_data_path`)
	h.WebkitDownloadPath = ConfigViper.GetString(`path.webkit_download_path`)
