...
func NewLocator(locators string, page *playwright.Page, elementOp *ElementOp, log *gstool.GsSlog) *Locator {
	return &Locator{
		Locators:  locators,
		Page:      page,
		ElementOp: elementOp,
...
func (h *Locator) parseLocator(Locator string) *LocatorParse {
	sList := strings.Split(Locator, `|`)
	locator := LocatorParse{
		Locator: sList[0],
		First:   false,
	}
